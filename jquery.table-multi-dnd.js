...
jQuery.tableDnD = {
            ...
            rows.each(function() {
                ...
                if (! row.hasClass("nodrag")) {
                    row.mousedown(function(ev) {
                        if (config.checkableClass && row.find('.'+config.checkableClass).prop('checked')) {
    ...
    mousemove: function(ev) {
        ...
        if (y != jQuery.tableDnD.oldY) {
            ...
            if (dragObjs.length != 0) {
                dragObjs.each(function(i, row) { moveRow(mousePos.y-row.mouseOffset.y, movingDown, dragObj, row); });
            } else {
